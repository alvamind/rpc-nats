...
import { CreateFileOptions, ExtendedDMMF, ExtendedDMMFField, ExtendedDMMFModel, ExtendedDMMFSchemaArgInputType, ZodValidatorOptions } from './classes';
export type WriteStatements = (datamodel: ExtendedDMMF, writer: CreateFileOptions) => void;
...
export type ZodStringValidatorKeys = 'min' | 'max' | 'length' | 'email' | 'url' | 'uuid' | 'cuid' | 'regex' | 'startsWith' | 'endsWith' | 'trim' | 'datetime' | 'noDefault' | 'describe';
export type ZodNumberValidatorKeys = 'gt' | 'gte' | 'lt' | 'lte' | 'int' | 'positive' | 'nonpositive' | 'negative' | 'nonnegative' | 'multipleOf' | 'finite' | 'noDefault' | 'describe';
...
export type PrismaAction = 'findUnique' | 'findMany' | 'findFirst' | 'createOne' | 'createMany' | 'createManyAndReturn' | 'updateOne' | 'updateMany' | 'upsertOne' | 'deleteOne' | 'deleteMany' | 'executeRaw' | 'aggregate' | 'count' | 'groupBy';
