...
// src/utils.ts
function transformTRPCResponseItem2(router, item) {
  if ("error" in item) {
    return {
      ...item,
...
function transformTRPCResponse2(router, itemOrItems) {
  return Array.isArray(itemOrItems) ? itemOrItems.map((item) => transformTRPCResponseItem2(router, item)) : transformTRPCResponseItem2(router, itemOrItems);
}
...
function getTRPCErrorFromUnknown2(cause) {
  ...
  if (error.name === "TRPCError") return cause;
  const trpcError = new TRPCError({
    code: "INTERNAL_SERVER_ERROR",
    cause: error,
    message: error.message
...
function compile(schema) {
  const check = (0, import_elysia.getSchemaValidator)(schema, {});
...
var trpc = (router, { endpoint = "/trpc", ...options } = {
  endpoint: "/trpc"
}) => (eri) => {
  let app = eri.onParse({ as: "global" }, async ({ request: { url } }) => {
                ...
                JSON.stringify(
                  transformTRPCResponse2(router, {
                    ...
                    error: router.getErrorShape({
                      error: getTRPCErrorFromUnknown2(
                        err
                      ),
                      type: incoming.method,
                      path: incoming.params.path,
